...
class TextClassifier(nn.ModuleList):
   ...
   def init_hidden(self):
      ...
      h = torch.zeros((self.LSTM_layers, 1, self.hidden_dim), dtype=torch.float, requires_grad=True)
      c = torch.zeros((self.LSTM_layers, 1, self.hidden_dim), dtype=torch.float, requires_grad=True)
